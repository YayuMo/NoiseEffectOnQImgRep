...
def circuitEvaluation(imgPath, imgSizeList, encoding, backend):
    ...
    if encoding == 'Amplitude Encoding':
    ...
    elif encoding == 'Angle QRAM Encoding':
        for size in tqdm(imgSizeList):
            vec = image2Arr(imgPath, size)
            qc,n = angleQRAM(vec)
            qc_depths.append(qc.depth())
        ...
        for size in tqdm(imgSizeList):
            vec = imageOpen(imgPath, size, 'L')
            qc = FTQR(vec)
            qc_depths.append(qc.decompose().depth())
            qc_nums.append(qc.num_qubits)
...
if __name__ == '__main__':
    ...
    angleQram_depths, angleQram_nums = circuitEvaluation(imgPath, imgSizeList, encoding='Angle QRAM Encoding', backend=hardware)
    ...
    plt.subplot(1,2,1)
    plt.plot(imgSizeList, basis_depths, color = 'red', linestyle='-', label='Basis', marker='*')
    plt.plot(imgSizeList, angle_depths, color = 'blue', linestyle='-', label='Angle', marker='x')
    plt.plot(imgSizeList, dense_depths, color = 'purple', linestyle='-', label='Dense Angle', marker='D')
    plt.plot(imgSizeList, amp_depths, color = 'yellow', linestyle='-', label='Amplitude', marker='^')
    plt.plot(imgSizeList, quam_depths, color = 'green', linestyle='-', label='QUAM', marker='o')
    plt.plot(imgSizeList, qram_depths, color = 'black', linestyle='-', label='QRAM', marker='+')
    plt.plot(imgSizeList, angleQram_depths, color = 'brown', linestyle='-', label='AngleQRAM', marker='v')
    ...
    plt.subplot(1,2,2)
    plt.plot(imgSizeList, basis_nums, color='red', linestyle='-', label='Basis', marker='*')
    plt.plot(imgSizeList, angle_nums, color='blue', linestyle='-', label='Angle', marker='x')
    plt.plot(imgSizeList, dense_nums, color='purple', linestyle='-', label='Dense Angle', marker='D')
    plt.plot(imgSizeList, amp_nums, color='yellow', linestyle='-', label='Amplitude', marker='^')
    plt.plot(imgSizeList, quam_nums, color='green', linestyle='-', label='QUAM', marker='o')
    plt.plot(imgSizeList, qram_nums, color = 'black', linestyle='-', label='QRAM', marker='+')
    plt.plot(imgSizeList, angleQram_nums, color = 'brown', linestyle='-', label='AngleQRAM', marker='v')
